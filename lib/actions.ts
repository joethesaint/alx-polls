  ...
  try {
    ...
    if (optionsError) {
      ...
      return {
        ...
        errors: {
          root: {
            _errors: ["Failed to create poll options. Please try again."],
          },
        },
      };
    }
...
export async function updatePoll(pollId: string, formData: PollFormValues) {
  ...
  try {
    ...
    const { error: transactionError } = await supabase.rpc(
      "update_poll_with_options",
      {
        poll_id: pollId,
        poll_question: formData.question,
    ...
    if (transactionError?.code === "42883") {
      ...
      const [pollUpdateResult, optionsDeleteResult] =
      ...
      if (pollUpdateResult.error) {
        ...
        return {
          success: false,
          errors: {
            root: { _errors: ["Failed to update poll. Please try again."] },
          },
        };
      }
      ...
      if (optionsDeleteResult.error) {
        console.error(
          "Error deleting poll options:",
          optionsDeleteResult.error,
        );
        return {
          success: false,
          errors: {
            root: {
              _errors: ["Failed to update poll options. Please try again."],
            },
          },
        };
      }
